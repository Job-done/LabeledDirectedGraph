...
var force = d3.layout.force()
    .nodes(challenge.ScalaJSExample().nodes)
    .links(challenge.ScalaJSExample().links)
    .size([width, height])
    .linkDistance(150)
    .charge(-500)
    .on('tick', tick);
...
function restart() {
    ...
    circle = circle.data(challenge.ScalaJSExample().nodes, function (d) {
        return d.id;
    });
...
function spliceLinksForNode(node) {
    var toSplice = challenge.ScalaJSExample().links.filter(function (l) {
        return (l.source === node || l.target === node);
    });
    toSplice.map(function (l) {
        challenge.ScalaJSExample().links.splice(challenge.ScalaJSExample().links.indexOf(l), 1);
    });
}
